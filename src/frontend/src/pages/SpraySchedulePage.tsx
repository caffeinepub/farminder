...
import type { Date_, SpraySchedule } from "../backend.d";
import {
  useAddSpraySchedule,
...
export default function SpraySchedulePage() {
  ...
  const [rows, setRows] = useState<SprayRow[]>([emptyRow()]);

  const { data: schedules, isLoading: schedulesLoading } =
    useGetSpraySchedulesForMonth(viewMonth, viewYear);
  const { data: crops, isLoading: cropsLoading } = useListCrops();
  const { mutateAsync: addSchedule, isPending: adding } = useAddSpraySchedule();
  ...
  const scheduleMap = new Map<number, SpraySchedule[]>();
  ...
  const isFormValid = scheduleDay && rows.every((r) => r.cropId && r.sprayName);
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(
        rows.length > 1
          ? `${rows.length} spray schedules added!`
          : "Spray schedule added!",
      );
      setAddOpen(false);
      setRows([emptyRow()]);
      setScheduleDay("");
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-muted-foreground">
              Plan your monthly crop spraying and chemical applications.
            </p>
          </div>
          <Button
            data-ocid="spray.open_modal_button"
            onClick={() => setAddOpen(true)}
            className="rounded-full bg-blue-600 hover:bg-blue-700 text-white"
          >
            <Plus className="w-4 h-4 mr-2" /> Add Spray Schedule
          </Button>
        </div>
            ...
            <Card className="shadow-card">
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  <button
                    type="button"
                    data-ocid="spray.pagination_prev"
                    onClick={prevMonth}
                  ...
                  <button
                    type="button"
                    data-ocid="spray.pagination_next"
                    onClick={nextMonth}
                    className="p-2 rounded-lg hover:bg-muted transition-colors"
                  >
                              ...
                              <>
                                ...
                                <div className="space-y-0.5">
                                  {events.slice(0, 2).map((ev) => (
                                    <div
                                      key={ev.id.toString()}
                                      className="bg-blue-500 rounded text-white text-[9px] px-1 py-0.5 truncate"
                                    >
                                      {ev.sprayName}
                                    </div>
                                  ))}
              ...
              <CardContent>
                {!selectedDay ? (
                  <p className="text-sm text-muted-foreground">
                    Click on a day to see scheduled spray tasks.
                  </p>
                ) : daySchedules.length === 0 ? (
                  <div
                    data-ocid="spray.empty_state"
                    className="text-center py-6"
                  >
                    <Droplets className="w-8 h-8 text-muted-foreground mx-auto mb-2" />
                    <p className="text-sm text-muted-foreground">
                      No spray tasks this day
                    </p>
                    <Button
                      size="sm"
                      variant="outline"
                        ...
                        >
                          <div className="flex items-start justify-between gap-2">
                            ...
                            {s.isDone ? (
                              ...
                              <Badge
                                variant="outline"
                                className="shrink-0 text-xs text-blue-600 border-blue-400"
                              >
                                Pending
                              </Badge>
                            )}
      ...
      >
        ...
        >
          ...
          <form onSubmit={handleAdd} className="flex flex-col gap-4 mt-2">
            <div>
              <Label htmlFor="spray-day">Day of Month</Label>
              <Input
                id="spray-day"
                type="number"
                min={1}
            ...
            <div className="space-y-3">
              ...
              {rows.map((row, index) => (
                <div
                  ...
                  data-ocid={`spray.item.${index + 1}`}
                >
                  {rows.length > 1 && (
                    <button
                      type="button"
                      data-ocid={`spray.delete_button.${index + 1}`}
                  ...
                  {cropsLoading ? (
                    ...
                    <div>
                      ...
                      >
                        <SelectTrigger data-ocid="spray.select">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      value={row.sprayName}
                      onChange={(e) =>
                        updateRow(row.id, "sprayName", e.target.value)
                      }
            ...
            <div className="flex gap-3 mt-1">
              ...
              >
                ...
                {rows.length > 1 ? `Add ${rows.length} Sprays` : "Add Spray"}
              </Button>
            </div>
          </form>
        </DialogContent>
